...
- For house roofs with visible clay/brick tiles, use module or Array-style repetition for tile rows by default, with editable spacing and rows. Do not flatten tile roofs into a single painted plane when the reference clearly depends on repeated tiles.
